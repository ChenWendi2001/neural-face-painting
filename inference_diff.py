...
from strokes import morphology, network
...
def param2stroke(param, H, W, meta_brushes, painter):
    """
...
def param2img_parallel(param, decision, meta_brushes, cur_canvas, painter):
    """
    ...
    valid_foregrounds, valid_alphas = param2stroke(param[decision, :], patch_size_y, patch_size_x, meta_brushes, painter)
...
def main(input_path, model_path, output_dir, resize_h, resize_w, increasing_layers):
    ...
    output_path =  os.path.join(output_dir, "strokes_" + input_name)
    patch_size = 32
    stroke_num = 8
    ...
    brush_large_vertical = read_img('strokes/brush/brush_large_vertical.png', 'L').to(device)
    brush_large_horizontal = read_img('strokes/brush/brush_large_horizontal.png', 'L').to(device)
    ...
    with torch.no_grad():
        ...
        for layer in range(0, K + 1):
            ...
            if increasing_layers is not None \
                and layer >= K - increasing_layers:
        ...
        pickle.dump(strokes, open(os.path.join(output_dir, "strokes.pkl"), "wb"))
