...
class Render(nn.Module):
    def __init__(self, strokes_file="strokes.pkl"):
        super().__init__()

        self.strokes = pickle.load(open(strokes_file, "rb"))
        self.patch_size = 32
        self.original_h = 512
        self.original_w = 512
        ...
        params = []
        for layer in range(0, self.K + 2):
            param, decision = self.strokes[layer]
            params.append(Stroke(param))
        
        self.parmas = nn.ModuleList(params)
    ...
    def forward(self):     
        ...
        for layer in range(0, self.K + 1):
            _, decision = self.strokes[layer]
            param = self.parmas[layer]()
